...
function useDetarutaData({idRuta}) {
...
//DEL
  ...
  const fetchData= async ()=> {
    const endpoint = `${API_URL}/clienteRutas/ruta/${idRuta}`;
    try {
      ...
      if (response.ok) {
        ...
        const dataWithActions = fetchedData.map(item => ({
          ...
          ruc: item.ruc,
          nombre: item.nombreComercial || item.nombre,
          mail: item.mail,
          telefono: item.telefono||"ND",
          action: (
              <>
                    <SoftTypography
                    component="a"
  ...
  useEffect(() => {
       fetchData();
    }, [idRuta]); // Empty dependency array to run the effect only once
  useEffect(() => {
...
export default useDetarutaData;